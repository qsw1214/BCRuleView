...
@interface BCRuleView()<UIScrollViewDelegate>
...
@property (nonatomic ,assign) BCRuleType ruleType;

@end

@implementation BCRuleView
...
-(instancetype)initWithFrame:(CGRect)frame{
    return [self initWithFrame:frame ruleType:BCRuleTypeHorizontal];
}
...
-(void)setRangeFrom:(NSInteger)minVale to:(NSInteger)maxValue minScale:(CGFloat)minScale minScaleWidth:(CGFloat)minScaleWidth{
    ...
    if (self.ruleType == BCRuleTypeVertical) {
    ...
    BCRuleContentView *contentView = [[BCRuleContentView alloc]initWithFrame:contentViewFrame
                                                                    minScale:minScale
                                                               minScaleWidth:minScaleWidth
                                                                     minVale:minVale
                                                                    maxValue:maxValue
                                                                  startIndex:self.ruleType == BCRuleTypeVertical ? self.bounds.size.height/2.0 : self.bounds.size.width/2.0
